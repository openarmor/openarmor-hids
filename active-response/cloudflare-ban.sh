...
# Path: /var/openarmor/active-response/bin/cloudflare-ban.sh
# Authors: Bryan Dwyer (bdtech) and Almir Bolduan (almirb)
# Last modified: May 08, 2019
#
# Sample config (Edit /var/openarmor/etc/openarmor.conf):
#
#  <command>
...
# Logging the call
echo "`date` $0 $1 $2 $3 $4 $5" >> /var/openarmor/logs/active-responses.log
...
if [ "x${ACTION}" = "xadd" ]; then
   ...
   --data "{\"mode\":\"$MODE\",\"configuration\":{\"target\":\"ip\",\"value\":\"$IP\"},\"notes\":\"Added via openarmor Command\"}"
   exit 0;
