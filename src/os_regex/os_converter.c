...
/* replacement map for openarmor regex to perl6 expressions */
...
int OSRegex_ConvertRegex(const char *pattern, char **converted_pattern_ptr)
{
    ...
    for (pattern_offset = 0UL; pattern_offset < pattern_size; pattern_offset++) {
        ...
        if (!replacement) {
            /* If it is a special class for openarmor */
     ...
     * Because openarmor is only greedy with the last occurencies modifier
     */
