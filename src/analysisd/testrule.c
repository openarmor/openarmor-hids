...
static void help_logtest(void)
{
    ...
    print_out("    -U <rule:alert:decoder>  Unit test. Refer to contrib/openarmor-testing/runtests.py");
    print_out(" ");
    exit(1);
}
...
int main(int argc, char **argv)
{
    ...
    if (gethostname(__shost, 512 - 1) != 0) {
        strncpy(__shost, openarmor_SERVER, 512 - 1);
    } else {
        char *_ltmp;
    ...
    {
        {
            ...
            if (!Config.decoders) {
                ...
                /* New loaded based on file specified in openarmor.conf */
                char **decodersfiles;
...
void OS_ReadMSG(char *ut_str)
{
    ...
    while (1) {
        ...
        if (fgets(msg + 8, OS_MAXSTR - 8, stdin)) {
            ...
            do {
                if (lf->decoder_info->type == openarmor_ALERT) {
