...
if [ "x$1" = "xlist" ]; then
    ...
    ls -la /etc/openarmor-init.conf > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        echo "ERROR: Unable to save rules. You must have openarmor installed to do so."
        exit 1;
    fi

    cat /etc/openarmor-init.conf > /dev/null 2>&1
    if [ ! $? = 0 ]; then
    ...
    ls -la /etc/openarmor-init.conf > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        echo "ERROR: Unable to restore rules. You must have openarmor installed to do so."
        exit 1;
    fi
