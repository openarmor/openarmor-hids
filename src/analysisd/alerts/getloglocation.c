...
int OS_GetLogLocation(const Eventinfo *lf)
{
    ...
    snprintf(__elogfile, OS_FLSIZE, "%s/%d/%s/openarmor-%s-%02d.log",
             EVENTS,
             lf->year,
             lf->mon,
             "archive",
    ...
    if (Config.logall_json) {
        ...
        snprintf(__ejlogfile, OS_FLSIZE, "%s/%d/%s/openarmor-%s-%02d.json",
                 EVENTS,
                 lf->year,
                 lf->mon,
                 "archive",
    ...
    /* Create the logfile name */
    snprintf(__alogfile, OS_FLSIZE, "%s/%d/%s/openarmor-%s-%02d.log",
             ALERTS,
             lf->year,
             lf->mon,
             "alerts",
    ...
    if (Config.jsonout_output) {
        ...
        snprintf(__jlogfile, OS_FLSIZE, "%s/%d/%s/openarmor-%s-%02d.json",
                 ALERTS,
                 lf->year,
                 lf->mon,
                 "alerts",
    ...
    /* Create the logfile name */
    snprintf(__flogfile, OS_FLSIZE, "%s/%d/%s/openarmor-%s-%02d.log",
             FWLOGS,
             lf->year,
             lf->mon,
             "firewall",
