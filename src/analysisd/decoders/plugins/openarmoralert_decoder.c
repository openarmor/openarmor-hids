...
/* openarmorAlert decoder init */
void *openarmorAlert_Decoder_Init()
{
    debug1("%s: Initializing openarmorAlert decoder.", ARGV0);
...
void *openarmorAlert_Decoder_Exec(Eventinfo *lf)
{
    ...
    lf->decoder_info->type = openarmor_ALERT;


    /* Checking the alert level. */
    if(strncmp("Alert Level: ", lf->log, 12) != 0 &&
       strncmp("openarmor: Alert Level:", lf->log, 18) != 0)
    {
        return(NULL);
    }
