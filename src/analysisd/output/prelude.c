...
#define DEFAULT_ANALYZER_NAME "openarmor"
#define ANALYZER_CLASS "Host IDS, File Integrity Checker, Log Analyzer"
#define ANALYZER_MODEL "openarmor"
#define ANALYZER_MANUFACTURER __site
#define ANALYZER_VERSION __openarmor_version

/** openarmor to prelude severity mapping. **/
static const char *(openarmor2prelude_sev[]) = {"info", "info", "info", "info",
                               "low", "low", "low", "low",
...
add_idmef_object(idmef_message_t *msg, const char *object, const char *value)
{
    ...
    if (ret < 0) {
        merror("%s: openarmor2Prelude: IDMEF: Cannot add object '%s': %s.",
...
err:
    merror("%s: openarmor2Prelude: %s: IDMEF error: %s.",
...
void OS_PreludeLog(const Eventinfo *lf)
{
    ...
    if ( ret < 0 ) {
        merror("%s: openarmor2Prelude: Cannot create IDMEF message", ARGV0);
        return;
    }
    ...
    add_idmef_object(idmef, "alert.assessment.impact.severity",
                     ...
                     openarmor2prelude_sev[lf->generated_rule->level]);
    ...
    /* Begin Classification Infomations */
    {
        ...
        /* Rule sid is used to create a link to the rule on the openarmor wiki */
        if (lf->generated_rule->sigid) {
            ...
            add_idmef_object(idmef, "alert.classification.reference(-1).meaning", "openarmor Rule Wiki Documentation");

            snprintf(_prelude_data, 256, "http://www.theopenarmor.org/wiki/Rule:%d",
         ...
         * that points back to the the openarmor wiki for more infomation.
         */
        if (lf->generated_rule->group) {
            ...
            while (copy_group) {
                ...
                add_idmef_object(idmef, "alert.classification.reference(-1).meaning", "openarmor Group Wiki Documentation");

                snprintf(_prelude_data, 256, "http://www.theopenarmor.org/wiki/Group:%s",
                         copy_group);
                add_idmef_object(idmef, "alert.classification.reference(-1).url", _prelude_data);
