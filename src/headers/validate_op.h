 ...
 * Returns 1 on success or 0 on failure
 */
int OS_IsonTime(const char *time_str, const char *openarmor_time) __attribute__((nonnull));

/* Same as above, but checks if time is the same or has passed a specified one */
int OS_IsAfterTime(const char *time_str, const char *openarmor_time) __attribute__((nonnull));

/** Day validations **/

/* Check if the specified week day is in the range */
int OS_IsonDay(int week_day, const char *openarmor_day) __attribute__((nonnull));
