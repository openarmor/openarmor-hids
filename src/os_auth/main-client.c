...
static void help_agent_auth()
{
    ...
    print_out("    -P <path>   Authorization password file [default: /var/openarmor/etc/authd.pass");
    print_out(" ");
    exit(1);
}
...
int main(int argc, char **argv)
{
    ...
    if (authpass) {
        snprintf(buf, 2048, "openarmor PASS: %s openarmor A:'%s'\n", authpass, agentname);
    }
    else {
        snprintf(buf, 2048, "openarmor A:'%s'\n", agentname);
    }
    ...
    while (1) {
        ...
        switch (SSL_get_error(ssl, ret)) {
            case SSL_ERROR_NONE:
                ...
                } else if (strncmp(buf, "openarmor K:'", 9) == 0) {
                    char *key;
                    char *tmpstr;
                    char **entry;
