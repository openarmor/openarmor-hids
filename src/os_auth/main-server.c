            ...
            if (FD_ISSET (sock, &fdwork)) {
                if ((client_sock = accept(sock, (struct sockaddr *) &_nc, &_ncl)) > 0) {
                    ...
                    if (pid) {
                        ...
                        if(authpass) {
                            /* Format is pretty simple: openarmor PASS: PASS WHATEVERACTION */
                            if (strncmp(tmpstr, "openarmor PASS: ", 12) == 0) {
                                tmpstr = tmpstr + 12;
                        ...
                        parseok = 0;
                        if (strncmp(tmpstr, "openarmor A:'", 9) == 0) {
                            agentname = tmpstr + 9;
                            tmpstr += 9;
                            while (*tmpstr != '\0') {
                                if (*tmpstr == '\'') {
                        ...
                        if (parseok == 0) {
                            ...
                            snprintf(response, 2048, "openarmor K:'%s'\n\n", finalkey);
