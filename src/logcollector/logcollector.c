...
void LogCollectorStart()
{
...
#else
            merror("%s: WARN: eventchannel not available on this version of openarmor", ARGV0);
#endif
...
#else
            merror("%s: WARN: journald not available on this version of openarmor", ARGV0);
#endif
            logff[i].command = NULL;
            logff[i].fp = NULL;
            logff[i].ptr = NULL;
...
#ifndef WIN32
            if (strcmp("openarmoralert", logff[i].logformat) == 0) {
                logff[i].read = read_openarmoralert;
            }
#endif
    ...
    while (1) {
        ...
        SendMSG(logr_queue, keepalive, "openarmor-keepalive", LOCALFILE_MQ);
		...
		if(tf == NULL) {
			...
			merror("Closing the temporary file %s did not work (%d): %s", logff[i].file, errno, strerror(errno));
...
#endif
                {
                    ...
                    snprintf(msg_alert, 512, "openarmor: File rotated (inode "
                             "changed): '%s'.",
                             logff[i].file);

                    /* Send message about log rotated */
                    SendMSG(logr_queue, msg_alert,
                            "openarmor-logcollector", LOCALFILE_MQ);
...
#endif
                {
                    ...
                    snprintf(msg_alert, 512, "openarmor: File size reduced "
                             "(inode remained): '%s'.",
                             logff[i].file);
