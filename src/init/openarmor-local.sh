...
NAME="openarmor HIDS"
VERSION="v3.8.0"
DAEMONS="openarmor-monitord openarmor-logcollector openarmor-syscheckd openarmor-analysisd openarmor-maild openarmor-execd ${DB_DAEMON} ${CSYSLOG_DAEMON} ${AGENTLESS_DAEMON}"
...
start()
{
    SDAEMONS="${DB_DAEMON} ${CSYSLOG_DAEMON} ${AGENTLESS_DAEMON} openarmor-maild openarmor-execd openarmor-analysisd openarmor-logcollector openarmor-syscheckd openarmor-monitord"
    ...
    ls -la "${DIR}/openarmor-agent/" >/dev/null 2>&1
    if [ $? = 0 ]; then
        echo ""
        echo "Starting sub agent directory (for hybrid mode)"
        ${DIR}/openarmor-agent/bin/openarmor-control start
    fi
...
stopa()
{
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/openarmor-agent/bin/openarmor-control stop
    fi
