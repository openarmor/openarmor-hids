...
static int _get_agent_os(const char *agent_name, const char *agent_ip, agent_info *agt_info)
{
    ...
    /* Get server info */
    if (!agent_name) {
        char *openarmor_version = NULL;
        agt_info->os = getuname();
        os_strdup(__openarmor_name " " __openarmor_version, agt_info->version);

        /* Remove newline */
        openarmor_version = strchr(agt_info->os, '\n');
        if (openarmor_version) {
            *openarmor_version = '\0';
        }

        openarmor_version = strstr(agt_info->os, " - ");
        if (openarmor_version) {
            *openarmor_version = '\0';
        }
    ...
    if (fgets(buf, 1024, fp)) {
        ...
        /* Remove newline */
        openarmor_version = strchr(buf, '\n');
        if (openarmor_version) {
            *openarmor_version = '\0';
        }

        openarmor_version = strstr(buf, " - ");
        if (openarmor_version) {
            *openarmor_version = '\0';
            openarmor_version += 3;

            os_calloc(1024 + 1, sizeof(char), agt_info->version);
            strncpy(agt_info->version, openarmor_version, 1024);
        }
