...
static const char *__gethour(const char *str, char *openarmor_hour) __attribute__((nonnull));
...
static char *_read_file(const char *high_name, const char *low_name, const char *defines_file)
{
    ...
    fp = fopen(def_file, "r");
    if (!fp) {
        if (strcmp(defines_file, openarmor_LDEFINES) != 0) {
...
int getDefine_Int(const char *high_name, const char *low_name, int min, int max)
{
    ...
    value = _read_file(high_name, low_name, openarmor_LDEFINES);
    if (!value) {
        value = _read_file(high_name, low_name, openarmor_DEFINES);
        if (!value) {
 ...
 * Returns 1 on success or 0 on failure
 */
int OS_IsonTime(const char *time_str, const char *openarmor_time)
{
    int _true = 1;
    ...
    if ((strncmp(time_str, openarmor_time, 5) >= 0) &&
            (strncmp(time_str, openarmor_time + 5, 5) <= 0)) {
        return (_true);
    }
...
/* Validate if a time is in an acceptable format for openarmor.
 * Returns 0 if doesn't match or a valid string for openarmor usage in success.
...
static const char *__gethour(const char *str, char *openarmor_hour)
{
    int _size = 0;
    int chour = 0;
    int cmin = 0;
    ...
    if ((*str == 'a') || (*str == 'A')) {
        str++;
        if ((*str == 'm') || (*str == 'M')) {
            snprintf(openarmor_hour, 6, "%02d:%02d", chour, cmin);
            str++;
            return (str);
        }
    } else if ((*str == 'p') || (*str == 'P')) {
...
int OS_IsAfterTime(const char *time_str, const char *openarmor_time)
{
...
int OS_IsonDay(int week_day, const char *openarmor_day)
{
    int _true = 1;
...
/* Validate if a day is in an acceptable format for openarmor
 * Returns 0 if doesn't match or a valid string for openarmor usage in success.
