...
void SendSecurityToken(const int socket, SecBuffer *OutBuffers)
{
    ...
    if (OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL)
    {
        ...
        if (sent <= 0)
            ErrorExit("%s: Could not send security token to server (is openarmor-authd running ?)", ARGV0);
...
void InstallAuthKeys(char *msg)
{
    ...
    else if (strncmp(msg, "openarmor K:'", 9) == 0)
    {
        char *key;
        char *tmpstr;
        char **entry;
...
int main(int argc, char **argv)
{
    ...
    if (authpass)
        SendSecureMessage(socket, &context, "openarmor PASS: %s openarmor A:'%s'\n", authpass, agentname);
    else
        SendSecureMessage(socket, &context, "openarmor A:'%s'\n", agentname);
