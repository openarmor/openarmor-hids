...
int gen_server_info(HWND hwnd)
{
    ...
    /* Set status data */
    SendMessage(hStatus, SB_SETTEXT, 0, (LPARAM)"http://www.theopenarmor.org");
...
/* Get openarmor Server IP */
int get_openarmor_server()
{
    OS_XML xml;
    char *str = NULL;

    /* Definitions */
    const char *(xml_serverip[]) = {"openarmor_config", "client", "server-ip", NULL};
    const char *(xml_serverhost[]) = {"openarmor_config", "client", "server-hostname", NULL};
...
/* Set openarmor Server IP */
int set_openarmor_server(char *ip, HWND hwnd)
{
    ...
    if (OS_IsValidIP(ip, NULL) != 1) {
        ...
        if (!s_ip) {
            MessageBox(hwnd, "Invalid Server.\r\n"
                       ...
                       "openarmor server or the resolvable hostname.",
    ...
    if (OS_WriteXML(CONFIG, tmp_path, xml_pt, NULL, ip) != 0) {
        MessageBox(hwnd, "Unable to set openarmor Server IP Address.\r\n"
...
/* Set openarmor Authentication Key */
int set_openarmor_key(char *key, HWND hwnd)
{
    FILE *fp;
