        ...
        __attribute__((unused))LPARAM lParam)
{
    ...
    switch (Message) {
        case WM_INITDIALOG: {
            ...
            AppendMenu(hSubMenu, menuflags, UI_MENU_MANAGE_START, "&Start openarmor");
            AppendMenu(hSubMenu, menuflags, UI_MENU_MANAGE_STOP, "&Stop openarmor");
            ...
            SendMessage(hwnd, WM_SETICON, ICON_SMALL,
                        (LPARAM)LoadIcon(GetModuleHandle(NULL),
                                         MAKEINTRESOURCE(IDI_openarmorICON)));
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                ...
                case IDC_ADD: {
                    ...
                    if (len > 0) {
                        ...
                        if (strcmp(buf, config_inst.server) != 0) {
                            if (set_openarmor_server(buf, hwnd)) {
                                chd = 1;
                            }
                        } else {
                            GlobalFree(buf);
                    ...
                    if (len > 0) {
                        ...
                        if (strcmp(buf, config_inst.key) != 0) {
                            ...
                            if (!ip) {
                                ...
                                if (ret == IDOK) {
                                    if (set_openarmor_key(decd_to_write, hwnd)) {
                                        chd += 2;
                                    }
                                }
                            }
                ...
                case UI_MENU_VIEW_LOGS:
                    _spawnlp( _P_NOWAIT, "notepad", "notepad " openarmorLOGS, NULL );
                    break;
                ...
                case UI_MENU_MANAGE_START:
                    ...
                    /* Start openarmor  -- must have a valid config before */
