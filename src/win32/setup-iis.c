...
int config_dir(char *name, char *dir, char *vfile)
{
    ...
    printf("%s: You may have it configured in a format different\n"
           ...
           "               log available.\n", name);
    printf("%s: http://www.theopenarmor.org/en/manual.html#iis\n\n", name);

    /* Add IIS config */
    fp = fopen(openarmorCONF, "a");
    if (!fp) {
...
int main(int argc, char **argv)
{
    ...
    /* Check if openarmor was installed already */
    if (!fileexist(openarmorCONF)) {
        printf("%s: Unable to find openarmor config: '%s'", argv[0], openarmorCONF);
        exit(0);
    }
    ...
    printf("%s: Looking for IIS log files to monitor.\r\n",
           argv[0]);
    printf("%s: For more information: http://www.theopenarmor.org/en/win.html\r\n",
           argv[0]);
    printf("\r\n");
