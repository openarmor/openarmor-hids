...
#########################################################  #
#                                                       #  #
# openarmor-batch-manager.pl - Add and extract agents from  #  #
# the openarmor client.keys file non-interactively. This    #  #
...
#$Id$
# TODO:
#   - Add check for openarmor 1.4 and support longer agent names
...
use constant AUTH_KEY_FILE => "/var/openarmor/etc/client.keys";
use constant RIDS_PATH => "/var/openarmor/queue/rids/";
...
# Adding a new agent
  ...
  if ($agentname && $ipaddress && 
      ...
      # openarmor doesn't like agent names > 32 characters.
      length($agentname) <= 32) {
