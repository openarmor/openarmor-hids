...
# ---------------------------------------------------------------------------
#
# "openarmor to Mysql" records the openarmor HIDS alert logs in MySQL database.
# It can run as a daemon (openarmor2mysqld.pl), recording in real-time the logs in database or
# as a simple script (openarmor2mysql.pl).
#
# ---------------------------------------------------------------------------
# Prerequisites
# ---------------------------------------------------------------------------
...
# 2a) Run openarmor2mysql.sql to create MySQL tables in your database
# 2b) Create BASE tables with snort tables extention
# 3) Create a user to access the database;
# 4) Copy openarmor2mysql.conf to /etc/openarmor2mysql.conf with 0600 permissions
# 3) Edit /etc/openarmor2mysql.conf according to your configuration:
#	dbhost=localhost
#	database=openarmorbase
#	debug=5
#	dbport=3306
#	dbpasswd=mypassword
#	dbuser=openarmoruser
#	daemonize=0
#	resolve=1
#	
#
...
my ($RUNASDAEMON)=0;
my ($DAEMONLOGFILE)='/var/log/openarmor2mysql.log';
my ($DAEMONLOGERRORFILE) = '/var/log/openarmor2mysql.err';
my ($LOGGER)='openarmor2mysql';
use openarmormysql;
...
&help() unless @ARGV;
my $dump=0;
my ($hids_id,$hids,$hids_interface,$last_cid)=(undef, 'localhost', 'openarmor',0);
...
while (@ARGV){
	...
	if (m/^-d$|^--daemon$/){
		$conf{daemonize}=1;
                ...
                $conf{hids_interface}= shift @ARGV if @ARGV; # openarmor-rt/openarmor-feed
        }elsif ( m/^--sensor$/){
...
my $dbi= openarmormysql->new(%conf) || die ("Could not connect to $conf{dbhost}:$conf{dbport}:$conf{database} as $conf{dbpasswd}\n");
####
# SQL vars;
...
my $delta=0;
########################################################
my $datepath=`date "+%Y/%b/openarmor-alerts-%d.log"`;
my $LOG='/var/openarmor/logs/alerts/'. $datepath;
chomp $LOG;
...
sub taillog {
   ...
   while (1==1) {
	...
	%resolv=();
       $| = 1;
       $stall += 1;
	$datepath=`date "+%Y/%b/openarmor-alerts-%d.log"`;
	$LOG='/var/openarmor/logs/alerts/'. $datepath;
	chomp $LOG;
...
sub version(){
	print "openarmor report tool $VERSION\n";
...
sub help(){
	&version();
	print "This tool helps you import into base the alerts generated by openarmor."
	...
	print "\t-n|--noname\n";
	print "\t-v|--verbose\n";
	print "\t--conf <openarmor2based-config>\n";
...
sub daemonize {
        ...
        if ($pid){
                open (PIDFILE , ">/var/run/openarmor2base2.pid") ;
                print PIDFILE "$pid\n";
                close (PIDFILE);
                exit 0;
        }
