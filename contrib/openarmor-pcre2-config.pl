...
my $openarmor_regex_convert = realpath(dirname($0) . '/../src/openarmor-regex-convert');

sub get_install_dir () {
    open(FILE, '<', 'src/LOCATION') || die("Cannot find INSTALL DIR");
    my $dir = '/var/openarmor';
...
sub convert_file ($) {
    ...
    my ($tmp_fh, $tmp_filename) = tempfile('tmp-openarmor-config-convert.XXXXX', DIR => '/tmp', SUFFIX => '.xml');

    while (<SRC>) {
        if (m{^(\s*)<\s*($old_tags)([^>]*)>(.*?)<\s*/\s*\g2\s*>}pg) {
            my ($indent, $old_type, $options, $old_regex) = ($1, $2, $3, $4);
            $old_regex =~ s/'/'\\''/g;
            my $out = qx/$openarmor_regex_convert -b -- $old_type '$old_regex'/;
            chomp($out);
...
if (! -d ${INSTALL_DIR}) {
    print(STDERR "Please install openarmor first\n");
    exit(1);
}
