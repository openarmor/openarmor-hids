...
# Licensed under GPL
my $VERSION='0.1';
my $openarmor_path='/var/openarmor';
my $rules_config="$openarmor_path/etc/rules_config.xml";
my $usersignatures_path="$openarmor_path/user_signatures";
my $signatures_path="$openarmor_path/signatures";
while ( @ARGV) {
	$_=shift @ARGV;
...
sub help(){
	...
	. "<rules>\n"
	. "\t<include>openarmor_rules.xml</include>"
 	."</rules>"
