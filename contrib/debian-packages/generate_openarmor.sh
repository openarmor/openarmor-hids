...
openarmor_version='3.6.0'
source_file="openarmor-hids-${openarmor_version}.tar.gz"
#packages=(openarmor-hids openarmor-hids-agent) # only options available
packages=(openarmor-hids-agent)
...
logfile=$scriptpath/openarmor_packages.log
...
{ 
  echo "
  This tool can be used to generate openarmor packages for Ubuntu and Debian.

  CONFIGURATION: The script is currently configured with the following variables:
    * Packages: ${packages[*]}. 
    * Distributions: ${codenames[*]}. 
    * Architectures: ${architectures[*]}.
    * openarmor version: ${openarmor_version}.
...
read_package_version()
{
  ...
  local regex="^openarmor-hids[A-Za-z-]* \([0-9]+.*[0-9]*.*[0-9]*-([0-9]+)[A-Za-z]*\)"
  while read line
  do
    if [[ $line =~ $regex ]]; then
...
update_changelog()
{
  ...
  local regex1="^(openarmor-hids[A-Za-z-]* \([0-9]+.*[0-9]*.*[0-9]*-[0-9]+)[A-Za-z]*\)"
...
download_source()
{
  ...
  do
    if [ ! -d ${debian_files_path}/${openarmor_version}/$package/debian ]; then
      echo "Error: Couldn't find debian files directory for $package, version ${openarmor_version}" | write_log
      exit 1
    fi
  done

  # Downloading file
  if wget -O $scriptpath/${source_file} -U openarmor https://github.com/openarmor/openarmor-hids/archive/${openarmor_version}.tar.gz ; then
    echo "Successfully downloaded source file ${source_file} from theopenarmor.org" | write_log
  else
  ...
  do
    ...
    cp -pr $scriptpath/${tmp_directory} $scriptpath/$package/$package-${openarmor_version}
    cp -p $scriptpath/${source_file} $scriptpath/$package/${package}_${openarmor_version}.orig.tar.gz
    cp -pr ${debian_files_path}/${openarmor_version}/$package/debian $scriptpath/$package/${package}-${openarmor_version}/debian
  done
  rm -rf $scriptpath/${tmp_directory}

  echo "The packages directories for ${packages[*]} version ${openarmor_version} have been successfully prepared." | write_log
}
...
do 
  ...
  do
    ...
    do
      ...
      local source_path="$scriptpath/${package}/${package}-${openarmor_version}"
      local changelog_file="${source_path}/debian/changelog"
      if [ ! -f ${changelog_file} ] ; then
        echo "Error: Couldn't find changelog file for ${package}-${openarmor_version}" | write_log
        exit 1
      fi
      ...
      local deb_file="${package}_${openarmor_version}-${package_version}${codename}_${arch}.deb"
      local changes_file="${package}_${openarmor_version}-${package_version}${codename}_${arch}.changes"
      local dsc_file="${package}_${openarmor_version}-${package_version}${codename}.dsc"
...
do
  ...
  do
    ...
    do
      ...
      cd ${results_dir}
      echo "Uploading package ${changes_file} for ${codename} to openarmor repository" | write_log
      if sudo /usr/bin/dupload --nomail -f --to openarmor-repository ${changes_file} ; then
        echo " + Successfully uploaded package ${changes_file} for ${codename} to openarmor repository" | write_log
      else
      ...
      # Moving package to the right directory at the openarmor apt repository server
