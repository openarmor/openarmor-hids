...
if ! [ -e /etc/openarmor-init.conf ]; then
    echo openarmor Manager not found. Exiting...
    exit 1
fi
...
if [ "X$FILE" = "X" ]; then
    ...
    echo "Example: $0 adddns theopenarmor.org"
...
if [ $ACTION = "addfile" ]; then
    ...
    grep "$FILE" ${DIRECTORY}/etc/openarmor.conf > /dev/null 2>&1
    if [ $? = 0 ]; then
...
if [ $ACTION = "adddns" ]; then
   ...
   grep "host -W 5 -t NS $FILE" ${DIRECTORY}/etc/openarmor.conf >/dev/null 2>&1
   if [ $? = 0 ]; then
   ...
   " >> ${DIRECTORY}/etc/openarmor.conf || MYERR=1;
   ...
   <rule id=\"$FIRSTRULE\" level=\"0\">
     <if_sid>530</if_sid>
     <check_diff />
     <match>^openarmor: output: 'host -W 5 -t NS $FILE</match>
...
if [ $ACTION = "addsite" ]; then
   ...
   grep "lynx --connect_timeout 10 --dump $FILE" ${DIRECTORY}/etc/openarmor.conf >/dev/null 2>&1
   if [ $? = 0 ]; then
       echo "$0: Already configured for $FILE"
       exit 1;
   fi
   ...
   <rule id=\"$FIRSTRULE\" level=\"0\">
     <if_sid>530</if_sid>
     <check_diff />
     <match>^openarmor: output: 'lynx --connect_timeout 10 --dump $FILE</match>
     <description>DNS Changed for $FILE</description>
   </rule>
   </group>
   " >> ${DIRECTORY}/rules/local_rules.xml || MYERR=1;
