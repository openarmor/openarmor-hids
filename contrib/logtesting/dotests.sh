...
echo "Starting log unit tests (must be run as root and on a system with openarmor installed)."
...
for i in ./*/log; do
    ...
    cat $i | /var/openarmor/bin/openarmor-logtest 2>&1|grep -av openarmor-testrule |grep -aA 500 "Phase 1:" > ./tmpres
