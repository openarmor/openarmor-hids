...
class openarmorTester(object):
    def __init__(self):
        self._error = False
        self._debug = False
        self._quiet = False
        self._openarmor_conf = "/var/openarmor/etc/openarmor.conf"
        self._base_dir = "/var/openarmor/"
        self._openarmor_path = "/var/openarmor/bin/"
        self._test_path = "./tests"

    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/openarmor-logtest' % (self._openarmor_path), ]
        cmd += ['-q']
        if self._openarmor_conf:
